...
const tryOnGenerate_1 = require("../services/tryOnGenerate");
...
const uploadAccess_2 = require("../services/uploadAccess");
exports.tryOnRouter = (0, express_1.Router)();
/** 每日试衣免费次数 */
const TRYON_FREE_PER_DAY = 5;
const PRESET_BG_EXT = /\.(jpg|jpeg|png|webp|gif)$/i;
exports.tryOnRouter.get('/preset-backgrounds', (req, res) => {
    const dir = (0, imagesPath_1.getPresetBackgroundDir)();
    let list = [];
    try {
        if (fs_1.default.existsSync(dir) && fs_1.default.statSync(dir).isDirectory()) {
            const files = fs_1.default.readdirSync(dir).filter((f) => PRESET_BG_EXT.test(f));
            const baseUrl = '/images/' + imagesPath_1.PRESET_BG_DIR_NAME;
            list = files.map((f) => {
                const id = f.replace(/\.[^.]+$/, '');
                return {
                    id: id.replace(/\s+/g, '-'),
 ...
 */
exports.tryOnRouter.post('/generate', async (req, res) => {
    try {
        ...
        const remaining = (0, dailyQuota_1.getTryonRemainingToday)(uid);
        if (remaining <= 0) {
        ...
        const outfit = oid ? init_1.db.prepare('SELECT id, name, need_points, image_url FROM outfits WHERE id = ?').get(oid) : undefined;
        const wardrobeItem = wid ? init_1.db.prepare('SELECT id, user_id, name, image_url FROM user_wardrobe_items WHERE id = ?').get(wid) : undefined;
        if (!outfit && !wardrobeItem)
            return res.status(404).json({ error: '搭配或我的衣物不存在' });
        if (outfit && outfit.need_points > 0 && oid && !(0, unlocks_1.isUnlocked)(uid, oid)) {
        ...
        const override = [height_cm, weight_kg, body_type, extra_prompt].some((v) => v !== undefined)
            ? { height_cm, weight_kg, body_type, extra_prompt }
            : undefined;
        const promptResult = outfit && oid
            ? (0, prompt_1.buildTryOnPrompt)(uid, oid, override)
            : {
        ...
        if (photoUrl.startsWith('http')) {
            ...
            const filename = (0, uploadAccess_1.filenameFromPath)(photoUrl);
            if (!(0, uploadAccess_1.isOwnerOfUpload)(uid, filename)) {
                return res.status(403).json({ error: '无权使用该照片试衣' });
            }
            const token = (0, upload_1.createTryOnAccessToken)(filename);
        ...
        if (outfitImageRef) {
            if (outfitImageRef.startsWith('http')) {
                ...
                const filename = (0, uploadAccess_1.filenameFromPath)(outfitImageRef);
                if (!(0, uploadAccess_1.isOwnerOfUpload)(uid, filename)) {
        ...
        const threeViews = await (0, tryOnGenerate_1.generateTryOnThreeViews)({
            personPhotoUrl,
            outfitImageUrl,
        ...
        const stmt = init_1.db.prepare('INSERT INTO tryon_results (user_id, outfit_id, wardrobe_item_id, photo_url, front_url, side_url, back_url) VALUES (?, ?, ?, ?, ?, ?, ?)');
        ...
        (0, dailyQuota_1.consumeTryonQuota)(uid);
        const { occasion, weatherTemp, weatherDesc } = req.body;
        if (oid)
            (0, outfitRecords_1.addOutfitRecord)(uid, oid, { occasion, weatherTemp, weatherDesc });
        res.status(201).json({
            resultId,
            model: tryonModel,
...
exports.tryOnRouter.post('/record-download', (req, res) => {
    const { userId } = req.body;
    const uid = userId ? Number(userId) : null;
    if (!uid)
        return res.status(401).json({ error: '请先登录' });
    if (!(0, dailyQuota_1.consumeDownloadQuota)(uid)) {
        return res.status(403).json({
            error: '今日免费下载次数已用完',
            downloadRemaining: 0,
            downloadLimit: 5,
    ...
    (0, energy_1.addEnergy)(uid, 10);
    const row = init_1.db.prepare('SELECT is_member FROM users WHERE id = ?').get(uid);
...
exports.tryOnRouter.get('/models', async (req, res) => {
...
exports.tryOnRouter.get('/results/:resultId', (req, res) => {
    const resultId = Number(req.params.resultId);
    const row = init_1.db.prepare('SELECT * FROM tryon_results WHERE id = ?').get(resultId);
    if (!row)
        return res.status(404).json({ error: '生成结果不存在' });
    res.json(row);
});
/**
 * 替换背景：试衣图 + 背景图融合为一张图。
 ...
 */
exports.tryOnRouter.post('/replace-background', (req, res) => {
    const { resultId, backgroundUrl } = req.body || {};
    const rid = Number(resultId);
    if (!rid)
        return res.status(400).json({ error: '需要 resultId' });
    const row = init_1.db.prepare('SELECT id, front_url FROM tryon_results WHERE id = ?').get(rid);
    if (!row)
        return res.status(404).json({ error: '生成结果不存在' });
    ...
    const run = async () => {
        ...
        if (!frontRes.ok)
            throw new Error('读取试衣图失败');
        const frontBuf = await frontRes.arrayBuffer();
        const readMaybeDataUrl = async (ref) => {
            if (ref.startsWith('data:')) {
                const idx = ref.indexOf(',');
                if (idx <= 0)
                    throw new Error('背景 data URL 无效');
            ...
            if (!res.ok)
                throw new Error(`读取背景图失败(${res.status})`);
            const arr = await res.arrayBuffer();
            return Buffer.from(arr);
        };
        const bgBuf = await readMaybeDataUrl(backgroundUrl);
        const front = await jimp_1.Jimp.read(Buffer.from(frontBuf));
        const bg = await jimp_1.Jimp.read(bgBuf);
        ...
        bg.brightness(-0.06);
        bg.composite(fg, 0, 0);
        if (!fs_1.default.existsSync(uploadAccess_2.UPLOAD_DIR))
            fs_1.default.mkdirSync(uploadAccess_2.UPLOAD_DIR, { recursive: true });
        const outName = `merged_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.jpg`;
        const outPath = path_1.default.join(uploadAccess_2.UPLOAD_DIR, outName);
 ...
 */
exports.tryOnRouter.post('/:resultId/beautify', (req, res) => {
    const resultId = Number(req.params.resultId);
    const { type } = req.body || {}; // 如 'skin_smooth' | 'color_tune'
    const row = init_1.db.prepare('SELECT id, front_url, side_url, back_url FROM tryon_results WHERE id = ?').get(resultId);
    if (!row)
        return res.status(404).json({ error: '生成结果不存在' });
    res.status(501).json({
        error: '美化功能待实现',
