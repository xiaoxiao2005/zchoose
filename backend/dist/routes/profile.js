 ...
 */
exports.profileRouter.get('/:userId/style', (req, res) => {
    const userId = Number(req.params.userId);
    const baseUrl = process.env.BASE_URL || `http://localhost:${process.env.PORT || 3001}`;
    const rows = init_1.db.prepare(`SELECT occasion, weather_desc, style_tags, name, image_ref, source, ts FROM (
     ...
     ORDER BY ts DESC`).all(userId, userId);
    const occasionCount = {};
    const tagCount = {};
    for (const r of rows) {
    ...
    const recentItems = rows.slice(0, RECENT_IMAGE_LIMIT).map((r) => ({
        name: r.name,
        kind: r.source === 'wardrobe' ? 'wardrobe' : 'outfit',
        imageUrl: (0, uploadAccess_1.accessUrlForStoragePath)(baseUrl, r.image_ref || null),
    }));
    res.json({
        totalRecords: total,
