...
exports.outfitsRouter = (0, express_1.Router)();
...
function shouldSyncCommuteMale25To29(tags) {
    if (!tags)
        return false;
...
function syncCommuteMale25To29Outfits() {
    ...
    const insertStmt = init_1.db.prepare('INSERT INTO outfits (name, image_url, style_tags, need_points) VALUES (?, ?, ?, ?)');
    const updateStmt = init_1.db.prepare('UPDATE outfits SET name = ?, style_tags = ? WHERE id = ?');
    const existsStmt = init_1.db.prepare('SELECT id FROM outfits WHERE image_url = ? LIMIT 1');
    for (let i = 0; i < files.length; i += 1) {
        ...
        const exists = existsStmt.get(imageUrl);
        if (exists) {
...
function syncCommuteMale30To35Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), COMMUTE_MALE_30_35_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncFestivalMale18To24Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), FESTIVAL_MALE_18_24_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncFestivalMale25To29Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), FESTIVAL_MALE_25_29_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncFestivalMale30To35Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), FESTIVAL_MALE_30_35_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncDatingMale18To24Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), DATING_MALE_18_24_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncDatingMale25To29Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), DATING_MALE_25_29_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncDatingMale30To35Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), DATING_MALE_30_35_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncSportMale25To29Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), SPORT_MALE_25_29_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncSportFemale25To29Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), SPORT_FEMALE_25_29_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
...
function syncDailyFemale18To24Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), DAILY_FEMALE_18_24_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
    ...
    const existingRows = init_1.db.prepare('SELECT id, image_url FROM outfits WHERE image_url LIKE ?').all(`/images/${DAILY_FEMALE_18_24_DIR_NAME}/%`);
    for (const row of existingRows) {
        if (!row.image_url || !validImageUrls.has(row.image_url)) {
            init_1.db.prepare('DELETE FROM outfits WHERE id = ?').run(row.id);
        }
    }
}
function syncDailyMale18To24Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), DAILY_MALE_18_24_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    // 先清洗历史命名，避免旧数据仍显示“去除图片左侧鞋子”
    init_1.db.prepare("UPDATE outfits SET name = REPLACE(name, '去除图片左侧鞋子', '') WHERE name LIKE '日常休闲-男-18-24-%去除图片左侧鞋子%'").run();
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
        .filter((entry) => !entry.name.includes('错误'))
        .map((entry) => entry.name)
    ...
    const existingRows = init_1.db.prepare('SELECT id, image_url FROM outfits WHERE image_url LIKE ?').all(`/images/${DAILY_MALE_18_24_DIR_NAME}/%`);
    for (const row of existingRows) {
        if (!row.image_url || !validImageUrls.has(row.image_url)) {
            init_1.db.prepare('DELETE FROM outfits WHERE id = ?').run(row.id);
        }
...
function syncDailyMale25To29Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), DAILY_MALE_25_29_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
    ...
    const existingRows = init_1.db.prepare('SELECT id, image_url FROM outfits WHERE image_url LIKE ?').all(`/images/${DAILY_MALE_25_29_DIR_NAME}/%`);
    for (const row of existingRows) {
        if (!row.image_url || !validImageUrls.has(row.image_url)) {
            init_1.db.prepare('DELETE FROM outfits WHERE id = ?').run(row.id);
        }
...
function syncZodiacFemale18To24Outfits() {
    const dir = path_1.default.join((0, imagesPath_1.getImagesDir)(), ZODIAC_DIR_NAME);
    if (!fs_1.default.existsSync(dir))
        return;
    const files = fs_1.default.readdirSync(dir, { withFileTypes: true })
        .filter((entry) => entry.isFile() && IMAGE_EXT_RE.test(entry.name))
    ...
    const insertStmt = init_1.db.prepare('INSERT INTO outfits (name, image_url, style_tags, need_points) VALUES (?, ?, ?, ?)');
    const updateStmt = init_1.db.prepare('UPDATE outfits SET name = ?, style_tags = ?, need_points = ? WHERE id = ?');
    const existsStmt = init_1.db.prepare('SELECT id FROM outfits WHERE image_url = ? LIMIT 1');
    for (let i = 0; i < files.length; i += 1) {
        const fileName = files[i];
...
function isPlaceholderOutfit(name) {
    if (!name || typeof name !== 'string')
        return false;
    if (name === '1-日常' || name === '2-rc')
        return true;
    // 示例搭配1～6 或 示例搭配一～六
    return /^示例搭配[一二三四五六123456]$/.test(name.trim());
}
// 衣库列表（可选：?tags=日常,通勤；?userId= 时每套返回 unlocked）
exports.outfitsRouter.get('/', (req, res) => {
    ...
    if (tags) {
        ...
        if (list.length) {
            ...
            rows = init_1.db.prepare(sql).all(...list);
        }
        else {
            rows = init_1.db.prepare(sql).all();
        }
    }
    else {
        rows = init_1.db.prepare(sql).all();
    ...
    if (!tags || (tags.split(',').map((t) => t.trim()).filter(Boolean).length === 0)) {
        rows = rows.filter((o) => !isPlaceholderOutfit(o.name));
    }
    if (userId != null) {
        rows = rows.map((o) => ({
            ...o,
            unlocked: (0, unlocks_1.isUnlocked)(userId, o.id),
            liked: (0, likes_1.isLiked)(userId, o.id),
        }));
    }
    const ids = rows.map((o) => o.id);
    let merchantSlotsByOutfit = {};
    if (ids.length > 0) {
        const placeholders = ids.map(() => '?').join(',');
        const slots = init_1.db.prepare(`SELECT oms.outfit_id, oms.slot, oms.merchant_id, m.name AS merchant_name, oms.product_url, oms.product_title
       FROM outfit_merchant_slots oms
       LEFT JOIN merchants m ON m.id = oms.merchant_id
       WHERE oms.outfit_id IN (${placeholders})`).all(...ids);
        slots.forEach((s) => {
            if (!merchantSlotsByOutfit[s.outfit_id])
                merchantSlotsByOutfit[s.outfit_id] = [];
...
exports.outfitsRouter.get('/:id', (req, res) => {
    const id = Number(req.params.id);
    const userId = req.query.userId ? Number(req.query.userId) : null;
    const row = init_1.db.prepare('SELECT * FROM outfits WHERE id = ?').get(id);
    if (!row)
        return res.status(404).json({ error: '搭配不存在' });
    if (userId != null) {
        row.unlocked = (0, unlocks_1.isUnlocked)(userId, id);
        row.liked = (0, likes_1.isLiked)(userId, id);
    }
    const slots = init_1.db.prepare(`SELECT oms.slot, oms.merchant_id, m.name AS merchant_name, oms.product_url, oms.product_title
     FROM outfit_merchant_slots oms
     LEFT JOIN merchants m ON m.id = oms.merchant_id
     WHERE oms.outfit_id = ?`).all(id);
    row.merchant_slots = slots.map((s) => ({
        slot: s.slot,
        merchant_id: s.merchant_id,
        merchant_name: s.merchant_name,
        product_url: s.product_url,
...
// 后台：新增一套搭配（后续可加鉴权）
exports.outfitsRouter.post('/', (req, res) => {
    const { name, image_url, style_tags, need_points } = req.body;
    if (!name)
        return res.status(400).json({ error: '请提供 name' });
    const tagsRaw = style_tags != null && String(style_tags).trim() !== '' ? (0, outfitImageTagger_1.sanitizeStyleTagsString)(String(style_tags)) : null;
    const stmt = init_1.db.prepare('INSERT INTO outfits (name, image_url, style_tags, need_points) VALUES (?, ?, ?, ?)');
    const result = stmt.run(name, image_url ?? null, tagsRaw && tagsRaw.length > 0 ? tagsRaw : null, need_points ?? 0);
    const row = init_1.db.prepare('SELECT * FROM outfits WHERE id = ?').get(result.lastInsertRowid);
    res.status(201).json(row);
});
// 后台：更新一套搭配（未传字段保持原值）
exports.outfitsRouter.put('/:id', (req, res) => {
    const id = Number(req.params.id);
    const { name, image_url, style_tags, need_points } = req.body;
    ...
    const tags = style_tags !== undefined
        ? (String(style_tags).trim() === '' ? null : (0, outfitImageTagger_1.sanitizeStyleTagsString)(String(style_tags)))
        : cur.style_tags;
    const pts = need_points !== undefined ? Number(need_points) : cur.need_points;
    init_1.db.prepare('UPDATE outfits SET name=?, image_url=?, style_tags=?, need_points=? WHERE id=?').run(n, img, tags, pts, id);
    const updated = init_1.db.prepare('SELECT * FROM outfits WHERE id = ?').get(id);
    res.json(updated);
});
// 后台：删除一套搭配
exports.outfitsRouter.delete('/:id', (req, res) => {
    const id = Number(req.params.id);
    const row = init_1.db.prepare('SELECT id FROM outfits WHERE id = ?').get(id);
    if (!row)
        return res.status(404).json({ error: '搭配不存在' });
    init_1.db.prepare('DELETE FROM outfits WHERE id = ?').run(id);
    res.status(204).send();
});
const VALID_SLOTS = ['上衣', '裤子', '鞋子', '配饰'];
function isMerchantUser(userId) {
    const row = init_1.db.prepare('SELECT role FROM users WHERE id = ?').get(userId);
    return row?.role === 'merchant';
}
/** 后台：为某套搭配添加上衣/裤子/鞋子槽位商家入驻（每槽最多 1 家，重复调用则更新） */
exports.outfitsRouter.put('/:id/merchant-slots', auth_1.requireAuth, (req, res) => {
    ...
    const outfit = init_1.db.prepare('SELECT id FROM outfits WHERE id = ?').get(outfitId);
    if (!outfit)
        return res.status(404).json({ error: '搭配不存在' });
    const merchant = init_1.db.prepare("SELECT id FROM merchants WHERE id = ? AND COALESCE(verification_status, 'approved') = 'approved'").get(merchantId);
    if (!merchant)
        return res.status(404).json({ error: '商家不存在' });
    const existing = init_1.db.prepare('SELECT id FROM outfit_merchant_slots WHERE outfit_id = ? AND slot = ?').get(outfitId, slot);
    if (existing) {
        init_1.db.prepare('UPDATE outfit_merchant_slots SET merchant_id = ?, product_url = ?, product_title = ?, created_at = datetime("now") WHERE outfit_id = ? AND slot = ?').run(merchantId, product_url ?? null, product_title ?? null, outfitId, slot);
    }
    else {
        init_1.db.prepare('INSERT INTO outfit_merchant_slots (outfit_id, slot, merchant_id, product_url, product_title) VALUES (?, ?, ?, ?, ?)').run(outfitId, slot, merchantId, product_url ?? null, product_title ?? null);
    }
    const updated = init_1.db.prepare('SELECT oms.*, m.name AS merchant_name FROM outfit_merchant_slots oms LEFT JOIN merchants m ON m.id = oms.merchant_id WHERE oms.outfit_id = ? AND oms.slot = ?').get(outfitId, slot);
    res.json(updated);
});
/** 后台：清空某套搭配的某个槽位（删除该槽位入驻） */
exports.outfitsRouter.delete('/:id/merchant-slots/:slot', auth_1.requireAuth, (req, res) => {
    const uid = Number(req.user?.userId || 0);
    if (!uid || !isMerchantUser(uid)) {
        return res.status(403).json({ error: '仅商家身份可操作' });
    }
    ...
    if (!outfit)
        return res.status(404).json({ error: '搭配不存在' });
    init_1.db.prepare('DELETE FROM outfit_merchant_slots WHERE outfit_id = ? AND slot = ?').run(outfitId, slot);
    res.status(204).send();
});
