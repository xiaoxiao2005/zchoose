...
const dbPath = process.env.DATABASE_PATH || path_1.default.join(__dirname, '../../data/app.db');
...
function runSchema(d) {
  ...
  `);
    ...
    const ensureOutfit = (name, imageUrl, styleTags, needPoints = 0) => {
    ...
    const seasonNow = monthNow >= 3 && monthNow <= 5 ? '春' : monthNow >= 6 && monthNow <= 8 ? '夏' : monthNow >= 9 && monthNow <= 11 ? '秋' : '冬';
    ...
    const commuteMale1822Dir = path_1.default.join(__dirname, '../../../frontend/public/images/职场通勤-男18-22岁（校园职场）');
    if (fs_1.default.existsSync(commuteMale1822Dir)) {
        const files = fs_1.default.readdirSync(commuteMale1822Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        // 用「文件名主干」命名，避免按序号重扫时同一文件被编成不同序号导致重复入库
        imageFiles.forEach((file) => {
            const stem = path_1.default.parse(file).name;
    ...
    const commuteFemale1824Dir = path_1.default.join(__dirname, '../../../frontend/public/images/职场通勤-女-18-24');
    if (fs_1.default.existsSync(commuteFemale1824Dir)) {
        const files = fs_1.default.readdirSync(commuteFemale1824Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file) => {
    ...
    const commuteFemale2529Dir = path_1.default.join(__dirname, '../../../frontend/public/images/职场通勤-女-25-29');
    if (fs_1.default.existsSync(commuteFemale2529Dir)) {
        const files = fs_1.default.readdirSync(commuteFemale2529Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file) => {
    ...
    const sportMale2529Dir = path_1.default.join(__dirname, '../../../frontend/public/images/运动出行-男-25-29');
    if (fs_1.default.existsSync(sportMale2529Dir)) {
        const files = fs_1.default.readdirSync(sportMale2529Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file) => {
    ...
    const festivalMale1824Dir = path_1.default.join(__dirname, '../../../frontend/public/images/节日家庭-男-18-24');
    if (fs_1.default.existsSync(festivalMale1824Dir)) {
        const files = fs_1.default.readdirSync(festivalMale1824Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
    ...
    const festivalMale2529Dir = path_1.default.join(__dirname, '../../../frontend/public/images/节日家庭-男-25-29');
    if (fs_1.default.existsSync(festivalMale2529Dir)) {
        const files = fs_1.default.readdirSync(festivalMale2529Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySpringMaleYouthDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-男-12-18');
    if (fs_1.default.existsSync(dailySpringMaleYouthDir)) {
        const files = fs_1.default.readdirSync(dailySpringMaleYouthDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySpringFemaleYouthDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-女-12-18');
    if (fs_1.default.existsSync(dailySpringFemaleYouthDir)) {
        const files = fs_1.default.readdirSync(dailySpringFemaleYouthDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailyFemale1824Dir = path_1.default.join(__dirname, '../../../frontend/public/images/日常休闲-女-18-24');
    if (fs_1.default.existsSync(dailyFemale1824Dir)) {
        const files = fs_1.default.readdirSync(dailyFemale1824Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file) => {
    ...
    const dailyFemale2529Dir = path_1.default.join(__dirname, '../../../frontend/public/images/日常休闲-女-25-29');
    if (fs_1.default.existsSync(dailyFemale2529Dir)) {
        const files = fs_1.default.readdirSync(dailyFemale2529Dir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file) => {
    ...
    const dailySpringFemaleAdultDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-女-18-35');
    if (fs_1.default.existsSync(dailySpringFemaleAdultDir)) {
        const files = fs_1.default.readdirSync(dailySpringFemaleAdultDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySpringFemaleMiddleDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-女-35-50');
    if (fs_1.default.existsSync(dailySpringFemaleMiddleDir)) {
        const files = fs_1.default.readdirSync(dailySpringFemaleMiddleDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySpringMaleAdultDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-男-18-35');
    if (fs_1.default.existsSync(dailySpringMaleAdultDir)) {
        const files = fs_1.default.readdirSync(dailySpringMaleAdultDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySummerMaleAdultDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-夏-男-18-35');
    if (fs_1.default.existsSync(dailySummerMaleAdultDir)) {
        const files = fs_1.default.readdirSync(dailySummerMaleAdultDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySummerFemaleAdultDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-夏-女-18-35');
    if (fs_1.default.existsSync(dailySummerFemaleAdultDir)) {
        const files = fs_1.default.readdirSync(dailySummerFemaleAdultDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySpringMaleMiddleDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-男-35-50');
    if (fs_1.default.existsSync(dailySpringMaleMiddleDir)) {
        const files = fs_1.default.readdirSync(dailySpringMaleMiddleDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const dailySpringMaleElderDir = path_1.default.join(__dirname, '../../../frontend/public/images/日常-春-男-50以上');
    if (fs_1.default.existsSync(dailySpringMaleElderDir)) {
        const files = fs_1.default.readdirSync(dailySpringMaleElderDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    const imagesBase = path_1.default.join(__dirname, '../../../frontend/public/images');
    const specialDirs = [
        { dir: '星座专属', tag: '星座专属' },
    ];
    specialDirs.forEach(({ dir, tag }) => {
        const fullPath = path_1.default.join(imagesBase, dir);
    ...
    const capricornDir = path_1.default.join(imagesBase, '星座专属穿搭', '摩羯女-夏季-日常');
    if (fs_1.default.existsSync(capricornDir) && fs_1.default.statSync(capricornDir).isDirectory()) {
        const files = fs_1.default.readdirSync(capricornDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
    ...
    try {
        d.run("UPDATE outfits SET style_tags = ? WHERE name LIKE '摩羯女-夏季-日常-%'", [tagCapricornNoDaily]);
    }
    catch (_) { /* 忽略 */ }
    // 统一规则：凡 style_tags 含「星座专属」的搭配均需积分解锁（含后续手动/API 新增）
    try {
        d.run(`UPDATE outfits SET need_points = ? WHERE style_tags LIKE '%星座专属%' AND (need_points IS NULL OR need_points = 0)`, [NEED_POINTS_SPECIAL]);
    }
    catch (_) { /* 忽略 */ }
    // 时装秀场：仅标签「时装秀场」；衣库页选该风格时不与其它维度组合（见前端 Outfits.tsx）
    const TAG_FASHION_SHOW = '时装秀场';
    const fashionShowDir = path_1.default.join(imagesBase, '时装秀场');
    if (fs_1.default.existsSync(fashionShowDir) && fs_1.default.statSync(fashionShowDir).isDirectory()) {
        const files = fs_1.default.readdirSync(fashionShowDir);
        const imageFiles = files
            .filter((f) => imageExts.some((ext) => f.toLowerCase().endsWith(ext)))
            .sort();
        imageFiles.forEach((file, i) => {
  ...
  `);
    ...
    merchantStmt.step();
    const merchantCount = merchantStmt.getAsObject().c;
    merchantStmt.free();
    if (merchantCount === 0) {
        ...
        midStmt.step();
        const mid = midStmt.getAsObject().id;
        midStmt.free();
...
exports.db = {
        ...
        return {
            run: (...params) => {
                ...
                try {
                    ...
                    if (stmt.step())
                        lastInsertRowid = Number(stmt.getAsObject().id);
                    stmt.free();
                }
                catch (_) { /* ignore */ }
    ...
    exec(_sql) {
        if (activeProvider === 'mysql')
            throw new Error('MySQL 模式下请使用 db.prepare(sql).run/get/all');
        getDb().exec(_sql);
    },
};
