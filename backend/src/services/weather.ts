...
function weatherFetchSignal(): AbortSignal {
  return AbortSignal.timeout(WEATHER_FETCH_TIMEOUT_MS);
}

/** Mock 数据（仅在所有 API 均失败时使用） */
function mockWeather(city?: string): { temp: number; desc: string; city: string } {
  return {
    temp: 22,
    desc: '晴',
    city: city || '北京',
 ...
 */
const WMO_TO_DESC: Record<number, string> = {
  0: '晴', 1: '晴', 2: '多云', 3: '阴',
  45: '雾', 48: '雾',
...
async function fetchOpenMeteo(city: string): Promise<{ temp: number; desc: string; city: string }> {
  const geoRes = await fetch(
    `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city)}&count=1&language=zh`,
    { signal: weatherFetchSignal() }
  );
  ...
  const forecastRes = await fetch(
    `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,weather_code&timezone=Asia/Shanghai`,
    { signal: weatherFetchSignal() }
  );
  ...
  if (!current) return mockWeather(city);
 ...
 */
async function fetchQWeather(city: string): Promise<{ temp: number; desc: string; city: string }> {
  const key = WEATHER_API_KEY!;
  const locationName = encodeURIComponent(city);
  const lookupRes = await fetch(
    `https://geoapi.qweather.com/v2/city/lookup?location=${locationName}&key=${key}`,
    { signal: weatherFetchSignal() }
  );
  const lookupData = (await lookupRes.json()) as { code?: string; location?: { id: string; name: string }[] };
  ...
  const nowData = (await nowRes.json()) as { code?: string; now?: { temp?: string; text?: string } };
 ...
 */
async function fetchGenericUrl(city: string): Promise<{ temp: number; desc: string; city: string }> {
  ...
  const now = data.now as { temp?: string; text?: string } | undefined;
 ...
 */
export async function getWeather(city?: string): Promise<{ temp: number; desc: string; city: string }> {
  const c = city || '北京';
