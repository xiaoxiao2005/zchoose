...
const THICKNESS_PRIORITY = ['厚实', '适中', '轻薄'] as const;
/** 中性色优先于暖/冷，减少「又暖又冷」残留 */
const TONE_TAGS = ['中性色', '暖色', '冷色'] as const;
const LIGHTNESS_TAGS = ['深色', '中明度', '浅色'] as const;
...
const DRESS_OVER_TOP_PRIORITY = ['连衣裙', '上装'] as const;
...
const COOL_HUES = [[70, 280]] as const;

const CATEGORY_RULES: Array<{ pattern: RegExp; tag: string }> = [
...
const THICKNESS_RULES: Array<{ pattern: RegExp; tag: string }> = [
...
function colorGroupFromHue(hue: number, sat: number, light: number): string {
  if (sat < 0.12) {
    if (light < 0.25) return '黑色系';
    if (light > 0.82) return '白色系';
    return '灰色系';
  }
  if (hue < 25 || hue >= 330) return '红色系';
  if (hue < 50) return '橙色系';
  if (hue < 70) return '黄色系';
  if (hue < 170) return '绿色系';
  if (hue < 260) return '蓝色系';
  if (hue < 300) return '紫色系';
  return '粉色系';
}

function toneFromHSV(hue: number, sat: number): string {
  if (sat < 0.12) return '中性色';
  if (inAnyRange(hue, WARM_HUES)) return '暖色';
  if (inAnyRange(hue, COOL_HUES)) return '冷色';
  return '中性色';
}

function lightnessTag(light: number): string {
  if (light < 0.35) return '深色';
  if (light > 0.72) return '浅色';
  return '中明度';
}

function tagsFromFileName(fileNameWithoutExt: string): string[] {
  const tags = new Set<string>();

  for (const rule of CATEGORY_RULES) {
    if (rule.pattern.test(fileNameWithoutExt)) tags.add(rule.tag);
  }
...
function keepOneByPriority(tags: Set<string>, ordered: readonly string[]): void {
 ...
 */
export function normalizeOutfitTags(input: string[]): string[] {
  const tags = new Set(
    input
      .map((t) => t.trim())
      .filter(Boolean)
 ...
 */
export function sanitizeStyleTagsString(raw: string | null | undefined): string {
  const parts = (raw || '')
    .split(',')
    .map((t) => t.trim())
    .filter(Boolean)
...
export async function analyzeOutfitImage(imageAbsPath: string, displayName: string): Promise<OutfitAutoTags> {
  ...
  const fileStem = path.parse(displayName).name;
  const nameTags = tagsFromFileName(fileStem);

  const tags = new Set<string>(nameTags);
