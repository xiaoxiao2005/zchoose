...
export interface TryOnPromptResult {
  fullPrompt: string;
  bodySnippet: string;
  outfitDesc: string;
...
export function buildTryOnPrompt(
  userId: number,
  outfitId: number,
  override?: { height_cm?: number; weight_kg?: number; body_type?: string; extra_prompt?: string }
): TryOnPromptResult | null {
  const body = db.prepare('SELECT gender, height_cm, weight_kg, body_type, prompt_snippet, extra_prompt FROM body_profiles WHERE user_id = ?').get(userId) as
    | { gender: string | null; height_cm: number | null; weight_kg: number | null; body_type: string | null; prompt_snippet: string | null; extra_prompt: string | null }
    | undefined;
  const outfit = db.prepare('SELECT name, style_tags, image_url FROM outfits WHERE id = ?').get(outfitId) as
    | { name: string; style_tags: string | null; image_url: string | null }
    | undefined;
  ...
  const parts: string[] = [];
  if (body?.gender) parts.push(`性别：${body.gender}`);
  const h = override?.height_cm ?? body?.height_cm;
  if (h != null) parts.push(`身高${h}cm`);
  const w = override?.weight_kg ?? body?.weight_kg;
  if (w != null) parts.push(`体重${w}kg`);
  if (bodyTypeLabel) parts.push(`体型：${bodyTypeLabel}`);
  if (bodySnippet) parts.push(bodySnippet);
